...
async function findVueNodes({page,pNode,tag,css,prop}){
	let jsHandle = await page.evaluateHandle(async (pNode,{tag,css,prop}) => {
		let el = window.__initPageGlobalFunctions.getElement(pNode)||document.body
		let elment = null
		if(css){
		...
		elment = [].slice.call(elment).map(e=>{
			return  window.__initPageGlobalFunctions.getComponentInstance(e)
		}).filter(e=>e)
		elment = Array.from(new Set(elment))
		if(tag){
			let newArr = []
			for(let e of elment){
				if(window.__initPageGlobalFunctions.getComponentName(e)===tag){
					newArr.push(e)
				}
			}
			elment = newArr
		...
		if(prop){
			elment = elment.filter(e=>{
				return Object.keys(prop).every(k=>e[k]===prop[k])
			})
		}
		return elment
	}, pNode,{tag,css,prop})
...
async function getLength({page,jsHandle,prop}){
	let value = await page.evaluate(async (jsHandle,prop) => {
		...
		let arr =  window.__initPageGlobalFunctions.getProp(jsHandle,prop)
...
async function getProp({page,jsHandle,prop}){
	let handle = await page.evaluateHandle((jsHandle,prop) => {
		...
		return window.__initPageGlobalFunctions.getProp(jsHandle,prop)
	}, jsHandle,prop)
...
async function setProp({page,jsHandle,prop,value}){
	...
	await page.evaluate(async (jsHandle,prop,value) => {
		...
		prop = Array.isArray(prop)?prop:[prop]
		jsHandle = jsHandle||window
		window.__initPageGlobalFunctions.setProp(jsHandle,prop,value)
...
async function doMethod({page,jsHandle,prop,arg,parentHandle}){
	...
	let handle = await page.evaluateHandle(async (jsHandle,prop,parentHandle,...arg) => {
		...
		prop = Array.isArray(prop)?prop:[prop]
		jsHandle = jsHandle||window
		return await window.__initPageGlobalFunctions.doMethod(jsHandle,prop,arg,parentHandle)
