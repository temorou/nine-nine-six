...
function initPageGlobalFunctions(){
   ...
   */
	...
	function upperCase(name) {
		...
		return name.replace(classifyRE, (_, c)=>{
   ...
   */
	function getComponentName (com) {
		...
		const name =  options._componentTag||options.name
		if (name) {
	...
	function deserializeFunction(value){
		...
		if(/^__serialize__[\d]+__function__$/.test(value)){
			return async function (...args) {
				...
				let res = await window.__callCacheMethod(value,...args)
				cacheMethodNumber--
				return res
			}
		}
